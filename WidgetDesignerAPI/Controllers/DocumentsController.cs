...
namespace WidgetDesignerAPI.API.Controllers
{
    ...
    public class DocumentsController : Controller
    {
        ...
        public async Task<bool> GetPDF(string filepath)
        {
            try
            {
                string fileName = Path.GetFileName(filepath); // Get the file name from the source file path
                string destinationPath = Path.Combine("E:\\QuikSyncProjects\\Dynamic Html creation\\", fileName); // Combine the destination folder and file name

                // Check if the source file exists
                if (System.IO.File.Exists(filepath))
                {
                    // Copy the file to the destination folder
                    System.IO.File.Copy(filepath, destinationPath, true); // The 'true' parameter overwrites the file if it already exists in the destination
                }
                else
                {
